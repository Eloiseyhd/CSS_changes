    ...
    RandomNumGenerator * rGen)
{
    ...
    // if(humans->empty()){return biteTaken;}
    std::set<Human *>::iterator itrHum = humans->begin();

    for(; itrHum != humans->end(); itrHum++){
        std::pair<std::string,double>* pairPtr = (*itrHum)->getCurrentLoc(time);
        if(pairPtr->first == locationID){
    ...
    //     biteTaken = 1;
    // }
// cout << "bite taken: " << biteTaken << endl;
    // if(!humanMap.empty())
    //     *biteTaken = 1;
...
// cout << mapItr->first->getHouseID() << " --- " << mapItr->first->getCurrentLoc(time)->first << " --- " << locationID << endl;
    // humBite = (--mapItr)->first;
        return mapItr->first;
}
